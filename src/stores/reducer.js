import { getTicketsListFS } from '../servises/fetch'
    ...
    switch (action.type){
        ...
        case 'updateActualTicketsList':{
            ...
            if (state.filters.sans===false){
                newActualTicketsList=newActualTicketsList.filter(ticket=>
            ...
            if (state.sort==='cheap'){
                newActualTicketsList=newActualTicketsList.sort((ticketTarget,ticketPosition)=>
                    ticketTarget.price-ticketPosition.price
                )
            }
            if (state.sort==='fast'){
                newActualTicketsList=newActualTicketsList.sort((ticketTarget,ticketPosition)=>
                    (ticketTarget.segments[0].duration+ticketTarget.segments[1].duration)
            ...
            if (state.sort==='optimal'){
                ...
                newActualTicketsList=newActualTicketsList.sort((ticketTarget,ticketPosition)=>
                        ...
                        -(
                            ...
                            +(ticketPosition.segments[0].duration+ticketPosition.segments[1].duration)/totalDuration
                            +(ticketPosition.segments[0].stops.length+ticketPosition.segments[1].stops.length)/totalStops
                        )
                )
            }
